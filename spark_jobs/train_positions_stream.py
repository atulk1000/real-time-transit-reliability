...
from transit_reliability.streaming.postgres_sink import process_train_position_envelopes


def main() -> None:
    bootstrap_servers = os.getenv("SPARK_KAFKA_BOOTSTRAP_SERVERS", "redpanda:9092")
    checkpoint_location = os.getenv(
        "SPARK_TRAIN_POSITIONS_CHECKPOINT",
    ...
    messages = (
        ...
        .option("subscribe", "wmata.train_positions.raw")
    ...
    def foreach_batch(batch_df, batch_id: int) -> None:
        ...
        with get_connection() as conn:
            process_train_position_envelopes(conn, envelopes, config)
            conn.commit()
